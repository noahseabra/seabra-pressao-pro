...
export async function analyzePressureImage(base64Image: string): Promise<AnalysisResult> {
  try {
    const response = await ai.models.generateContent({
      model: "gemini-3-flash-preview", // Multimodal model that supports JSON mode
      contents: [
        {
          parts: [
            {
    ...
    return { systolic: 0, diastolic: 0, error: "Falha na leitura da imagem. Verifique a iluminação e tente novamente." };
  }
}
